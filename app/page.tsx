...
export default function HomePage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.log('📡 Fazendo requisição para /api/gmail/simple-auth');
  ...
  const loadAnalytics = async (useMock = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(data.message || 'Analytics carregados com sucesso');
        console.log('✅ Analytics carregados:', data.data);
      } else {
        console.error('❌ Erro na resposta:', data.error);
        toast.error(data.error || 'Erro ao carregar analytics');
      }
    } catch (error) {
